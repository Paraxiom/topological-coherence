...
\textbf{Experimental validation}: In our Phi-2 (2.7B) experiments, a toroidal attention mask reduces drift rate by 40\% on synthetic sequences and achieves +19.5\% relative improvement on TruthfulQA (817 questions) with best-in-class HaluEval performance (52.60\% vs.\ 55.00\% baseline). On ByteDance's Ouro-1.4B Universal Transformer, toroidal logit bias at 4 recurrent loops achieves 90\% accuracy on factual QA, exceeding the 8-loop baseline (85\%)---demonstrating that geometric coherence can substitute for computational depth. Code, configurations, and logs are released at \url{https://github.com/Paraxiom/topological-coherence}.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Limited model diversity}: Validated on Phi-2 (2.7B, standard transformer) and Ouro-1.4B (Universal Transformer). Generalization to larger models (7B+) and other recurrent architectures remains to be demonstrated

    \item \textbf{Effect size varies by mechanism}: Attention-level toroidal injection shows modest gains (+0.40pp on HaluEval); logit-level TLB shows stronger effects (+10\% on Ouro). The optimal injection point depends on architecture

    \item \textbf{Small benchmark for Ouro results}: The 20-prompt factual QA benchmark, while validated across 64 hyperparameter configurations showing a robust improvement plateau, lacks the statistical power of larger benchmarks. TruthfulQA MC1 evaluation (200 questions) showed zero effect, likely because MC1 scoring uses mean-of-last-token logits (a discrimination task) rather than autoregressive generation---where TLB's logit bias operates. This distinction between generation-time and discrimination-time evaluation is an important methodological consideration for future work
\end{enumerate}
