...
The NLGS framework \citep{saezdeocariz2023nlgs} employs diffusion kernels based on graph Laplacian eigendecomposition $K = U e^{-\beta\Lambda} U^T$ to define similarity in their search space---the same spectral machinery underlying our toroidal approach. \citet{shuman2013emerging} established the foundations of signal processing on graphs, including spectral filtering via Laplacian eigenbases. Our spectral resonance extension (forthcoming) directly uses the Laplacian eigenbasis of the torus to compute harmonic mode energy for logit biasing.
